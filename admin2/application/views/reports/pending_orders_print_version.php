...
<div class="row">
  <div class="col-lg-12">
    <h4 class="print-sub-titles">Printed on: <?=date1(date('Y-m-d H:i:s'))?></h4>
    
    
    <h4 class="print-sub-titles">
      Period: 
    ...
    <? if(!intval($filters['plant'])): ?>         
      <? foreach($plants as $plant): ?>
        <h2 class="print-title"><?=t($plant['full_name'])?></h2>
        <hr />
        <? 
        ...
        <? else: ?>
          <? $this->load->view('reports/partials/orders-grid-print_version',['orders'=>$orders_filtered]); ?>
        <? endif; ?>
          
      <? endforeach; ?>
    <? else: ?> 
      <h3><?=t($selected_plant['full_name'])?></h3>
      <? $this->load->view('reports/partials/orders-grid-print_version',['orders'=>$orders])?>  
    <? endif; ?> 
