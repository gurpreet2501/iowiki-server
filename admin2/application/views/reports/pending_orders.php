...
<div class="row">
  <div class="col-lg-12">
    ...
    <div class="text-center">
      <a  target="__blank"
          href='<?=site_url('reports/pending_orders/print_version/?'.http_build_query(['filters'=>$filters]))?>' 
          class='btn btn-primary'>Print</a>
      <a  href='<?=site_url('reports/pending_orders/csv?'.http_build_query(['filters'=>$filters]))?>' 
    ...
    <? if(!intval($filters['plant'])): ?>         
      <? foreach($plants as $plant): ?>
        ...
        <? else: ?>
          <? $this->load->view('reports/partials/orders-grid',['orders'=>$orders_filtered]); ?>
        <? endif; ?>
          
      <? endforeach; ?>
    <? else: ?> 
      <? $this->load->view('reports/partials/orders-grid',['orders'=>$orders])?>  
    <? endif; ?> 
        ...
        <ul class="pager">
          <? if($orders['pagination']['previous']): ?>
            <?
              ...
              $prev_link = site_url('reports/pending_orders').'?'.http_build_query($prev_link);  
            ?>
          ...
          <? if($orders['pagination']['next']): ?>
            <?
              ...
              $next_link = site_url('reports/pending_orders').'?'.http_build_query($next_link);  
            ?>
