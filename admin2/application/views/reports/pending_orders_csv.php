...
if(!intval($filters['plant'])){
  foreach($plants as $plant){
    ...
    if(empty($orders_filtered['results']))
      ...
      $this->load->view('reports/partials/orders-grid-csv',['orders'=>$orders_filtered,'output'=>$output]);
  }
}else if(empty($orders['results']))
  fputcsv($output, ['Sorry, No orders found.']);  
else
  $this->load->view('reports/partials/orders-grid-csv',['orders'=>$orders,'output'=>$output]);
