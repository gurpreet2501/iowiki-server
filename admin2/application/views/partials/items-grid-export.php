...
<? endif; ?>
  <?php
    ...
    if($this->tank_auth->is_logged_in())
      $form_act = site_url('export/order');
  ?>
      ...
      <?php
       foreach($data as $key => $value):         ?>
          <tr class="back_orange">
            ...
            <td class="col-xs-1">
                <? if($value['image']): ?>
                    <img class="live_support pop_up_open_btn btn-lg" data-toggle="modal" data-target="#myModal-general-<?=$key?>" src="<?=base_url('/images/processing.gif')?>"  class="img-thumbnail img-fit-container" alt="" data-echo="<?=base_url('/timthumb.php?src=')?><?=base_url('assets/uploads/files/'.$value['image'].'&w=50&h=50')?>" />
                  <? else: ?>
             ...
             <td class="col-xs-3">
              <h5 class="text-center p_name">
               ...
               <div class="product_image_home">
                <div class="modal fade" id="myModal-general-<?=$key?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
             ...
             <td>
             	  USD $<span class="pprice-<?=$value['id']?>"><?=t($value['price'])?></span>
                <hr />
                <? if($show_stock): ?>
                ...
                <input type='hidden' name='products[<?=$value['id']?>][price]' value="<?=$value['price']?>" />
                <input type='hidden'  name="products[<?=$value['id']?>][plant_id]" value="<?=$value['plant_id']?>"/>
             </td>
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <span class="product_type">
            <input type="hidden" name="product_type" value="Export"/>
          </span>
    ...
    <div class='stick-wrap'>
      <div id="stick">
        <div class='text-center'>
          <strong>Total: $ <span class='order_total'>0.00</span></strong>
