...
use App\Models;
 ...
 $usr = Models\Users::select('role')->where('id', $this->tank_auth->get_user_id())->first();  
 if($usr->role == 'plant_manager')
...
<? if(user_role() == 'plant_manager'): ?>
  <div class="text-right desktop-only">
    <a  href="<?=site_url("manage/shipment/export/create/{$order['id']}")?>"
        class='btn btn-sm btn-success'>Update Shipment Progress</a>
  </div>
<? endif; ?>
